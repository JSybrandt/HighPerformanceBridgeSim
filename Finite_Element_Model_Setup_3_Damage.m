...
% Damage Variables
% THESE ARE NOW CONSTANT FOR THE WHOLE RUN

L    = BridgeVariables(1,1);
mu   = BridgeVariables(1,2);
E    = BridgeVariables(1,3);
I    = BridgeVariables(1,4);
beta = BridgeVariables(1,5);
W    = BridgeVariables(1,6);
...
parfor jjj = 1:lim % This is the loop for the number of days
    ...
    if RainEffects==1
        if jjj>=3
            ...
            elseif Rain(jjj)==0 && Rain(jjj-1)==1
                mu=mu-.001*rand(1,1)*BridgeVariables(Bridge,2);
            elseif Rain(jjj)==0 && mu>BridgeVariables(Bridge,2)
                mu=mu-.001*rand(1,1)*BridgeVariables(Bridge,2);
            elseif Rain(jjj)==0 && mu<=.99*BridgeVariables(Bridge,2)
    ...
    for ii=1:n % This is the loop for changing vehicle variables over time
        ...
        AB=zeros(2*NumberElements,1); % Initial global accelerations
        ...
        for i=1:Kt-1 % This loop is for calculating the accleration response for each vehicle crossing
            ...
            if Wind==1
                % FW(:)=ForceWind(jjj,ii);
                % Apply Numark Method
                RS=ForceWind(jjj,ii)-PC-QC-K*UB(:,i)-(-a1*M+C-a1*a4*C)*VB(:,i)-(-a2*M+a3*C-a4*a2*C)*AB(:,i);
                LS=(a0*M+a4*a0*C+K);
            else
                % Apply Numark Method
