function [VehicleFrequencyAmpData, VehicleFrequencyData, AccelerationVehicle, Time, RoadProfile, DerivativeRoadProfile, SecondDerivativeRoadProfile]=FEA_One_Vehicle
...
RC1=rc;
rch=KInsert2(rc_tdt_st,cor(j,:),2*(NumberElements+1));
RC1=RC1+rch;
end
...
if Surface==1
RC=zeros(2*NumberElements,1);
[PC,QC,RC]=bcloads(NumberElements,PC1,PC,QC1,QC,RC1,RC);
else
...
DerivativeRoadProfile{Day,ii}=drx;
SecondDerivativeRoadProfile{Day,ii}=ddrx;
end % end day loop
end
